...
1. 2014/11/18
	...
	1. 以下の処理を行うJavaクラス（クラス名：Select1.class)を作成
		...
		1. 以下を一覧表示する
			...
			1. loc(場所)
	1. Select1の改造<br /> 
上記のJavaクラスのデータベースへの問い合わせを以下通り変更<br />
executeQuery => prepareStatement  

1. 2014/11/19

	1. Select1の改造
		1. 上記の一覧表示後、empno(番号)を入力するプロンプトを表示
		1. 入力したempnoの以下を表示
		1. 社員番号、社員名、職種、上司の名前、部署名、部署のある場所、給与、給与の等級

1. 2014/11/20

	1. Select1の改造
		1. 上記に部下の社員番号、社員名、職種、部署名、部署のある場所、給与、給与の等級の一覧を追加する。部下がいない場合は「部下はいません」と表示する

1. 2014/11/25

	1. Delete1クラスの作成
		1. empno(番号)を入力するプロンプトを表示
		1. 入力したempnoの以下を表示
		1. 社員番号、社員名及び部下の名前
		1. 削除を行うかの確認
			1. 削除を行うを選択 => データを削除し、プログラムを終了する。但し、部下がいる場合は削除せず、削除できな旨表示
			1. 削除を行わないを選択 => プログラムを終了	 

1. 2014/11/26

	1. Update1クラスの作成 
		1. empno(番号)を入力するプロンプトを表示
		1. 入力したempnoの以下を１つずつ表示して、変更するか確認
			1. ename
			1. yomi
			1. job
			1. mgr
...
1. 2014/11/27
	...
	1. Insert1クラスの作成
		1. 追加したいデータの以下の項目を１つずつ入力させる。何も入力せずエンターのみ入力した場合はその項目はNULLとする
			1. ename
			1. yomi
			1. job
			1. mgr
